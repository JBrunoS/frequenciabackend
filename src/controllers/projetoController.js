...
module.exports = {
  ...
  async getLogin(req, res) {
    ...
    const projeto = await connection("projetos").select("*").where({
      email_projeto: email,
      senha_projeto: senha,
    });
  ...
  async create(req, res) {
    const { codigo_projeto, nome_projeto, email_projeto, senha_projeto } =
      req.body;
    ...
    if (projetos) {
      return res.json(
        "JÃ¡ existe um projeto usando esse email, insira um novo!",
      );
    } else {
